...
import { Plus, Edit, Trash2, Search, Linkedin, Twitter, Github, Loader2, Upload } from 'lucide-react';
    ...
    <div className="p-6">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingMember ? 'Edit Team Member' : 'Add Team Member'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-dark-300 mb-1">Photo</label>
            <div className="flex gap-2 items-center">
              ...
              />
              <label className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-dark-700 bg-dark-800 text-sm font-medium text-dark-200 hover:bg-dark-700 cursor-pointer transition-colors whitespace-nowrap ${uploadingPhoto ? 'opacity-50 pointer-events-none' : ''}`}>
                <Upload className="w-4 h-4 text-primary-400" />
                <span>{uploadingPhoto ? 'Uploading...' : 'Upload to MySQL'}</span>
                <input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
              </label>
            </div>
            {formData.photo && (
              <div className="mt-2 flex items-center gap-3 p-2 bg-dark-900/40 rounded-lg border border-dark-800">
                <img src={formData.photo} alt="Preview" className="w-10 h-10 object-cover rounded-full" />
                <span className="text-xs text-dark-400 truncate flex-1">{formData.photo}</span>
              </div>
            )}
          </div>
          <div>
